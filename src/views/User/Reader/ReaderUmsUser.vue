      ...
      <v-card>
        ...
        <v-card-text style="margin-top: 10px">
          <div>用户
            ...
            修改为：<br>
<!--            <el-select v-model="roleSelect" placeholder="请选择role" style="width: 250px">-->
<!--              <el-option-->
...
export default {
  ...
  methods: {
    async submit (){
      this.loading = true
      this.users = []
      await this.findAll('ds2_user').then(result => {
    ...
    async getRoles(){
      await this.findAll('ds2_role').then(result => {
        console.log(result)
    ...
    async setRole(){
      ...
      let refresh = false
      await this.updateUserRole(this.selectedUid, this.roleSelect, 'ds2').then(result =>{
        console.log(result)
        if (result){
    ...
    async addUser(){
      ...
      await this.addOneUser(this.userForm, 'ds2').then(result => {
        console.log(result)
        if (result.code === 200){
          this.$message({
            showClose: true,
    ...
    async deleteUser(){
      await this.deleteOneUser(this.deleteItem.uid, 'ds2')
      await this.submit()
